...
import { getSavedColumnPreferences } from '@/services/stockServices';
...
const MainStockView = () => {
  ...
  const { data: allStocks } = useSelector((state: RootState) => state.stocks);
  ...
  const activeTheme:Theme = themes['matte-black'];
  ...
  useEffect(() => {
    const loadSavedPreferences = async () => {
      if (firebaseIdToken) {
        ...
        try {
          const savedColumns = await getSavedColumnPreferences(firebaseIdToken);
          if (savedColumns.length > 0) {
            // Create new visible columns object with saved preferences
            ...
            savedColumns.forEach(columnKey => {
              if (columnKey in newVisibleColumns) {
                newVisibleColumns[columnKey] = true;
              }
            });
            
            // Ensure at least name column is always visible
            newVisibleColumns.name = true;
            
            setVisibleColumns(newVisibleColumns);
            toast.success('✅ Column preferences loaded');
          }
        } catch (error) {
  ...
  }, [firebaseIdToken, visibleColumns]);
  ...
  return (
    <div className="flex-1 z-0 overflow-hidden">
      {/* Header with controls - Responsive padding */}
      <div className="p-4 sm:p-6 lg:p-8 pb-0">
        <div className='flex flex-col sm:flex-row lg:justify-end w-full gap-2 sm:gap-4'>
          <SaveScreen
            theme={{
      ...
      {/* Table Container - Responsive padding and height */}
      <div className="px-4 sm:px-6 lg:px-8 pb-20 lg:pb-8 h-full overflow-hidden">
        <StockTable
          theme={activeTheme}
          ...
          visibleColumns={visibleColumns}
          sortedStockData={allStocks?.flat().slice(0, 50) || []}
          handleSort={() => {}}
          getSortIcon={() => null}
